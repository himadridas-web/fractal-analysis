...
with st.sidebar.form(key="text_input_form"):
    text = st.text_area(
        "Paste your text corpus here:",
        height=300,
...
# Main Execution
# -----------------------------
if submit_button or text.strip():
    words = preprocess(text)
    unique_words = len(set(words))

    if len(words) < 20:
        st.warning(
            "Please enter a longer text (at least 20 words) for meaningful fractal analysis."
        )
    elif unique_words < 2:
        st.error(
            "Your text needs at least two distinct unique words to calculate mathematical slopes."
        )
    else:
        st.write(
            f"### Total Corpus Size: **{len(words)}** tokens | Unique Vocabulary: **{unique_words}** tokens"
        )
        st.write("---")
        ...
        with col1:
            ...
            ranks, freqs, alpha, r2_zipf, log_ranks, intercept_zipf = zipf_analysis(
                words
            )
            ...
            m1.metric(
                "Zipf Exponent (α)",
                f"{alpha:.3f}",
                help="Standard natural language usually approaches 1.0",
            )
            m2.metric(
                "Fit Quality (R²)",
                f"{r2_zipf:.3f}",
                help="1.0 means perfect power-law (fractal) behavior.",
            )
            ...
            st.pyplot(fig1)
            plt.close(fig1)  # Free up memory

            st.info(
                "**What does this mean?** Zipf's law states that the frequency of any word is inversely proportional to its rank. In mathematics, this is a Power Law distribution. A straight line on a log-log plot indicates scale invariance—a core property of fractals."
            )
        ...
        with col2:
            ...
            (
                ...
                log_N,
                intercept_heaps,
            ) = heaps_scaling_analysis(words)

            m3, m4 = st.columns(2)
            m3.metric(
                "Scaling Exponent (β)",
                f"{beta:.3f}",
                help="Standard English typically ranges from 0.4 to 0.6",
            )
            m4.metric(
                "Fit Quality (R²)",
                f"{r2_heaps:.3f}",
                help="1.0 means perfect power-law scaling.",
            )
            ...
            st.pyplot(fig2)
            plt.close(fig2)  # Free up memory

            st.info(
                "**What does this mean?** This replaces box-counting. Heaps' law measures how vocabulary scales with text length. Just like measuring the length of a coastline depends on your 'measuring stick,' the size of your vocabulary depends on the size of your text sample following a power-law exponent."
            )
else:
